...
public class CarolBotExpert extends Player{
	...
	private boolean opponentNear, superCarol = false, opponentOnAir, opponentOnTop, opponentOnLeft, opponentOnRight, opponentAttacking, opponentShielding, opponentProjectileComing,
	...
	public CarolBotExpert(Game game, int playerNumb, Character character, double x, double y) {
	...
	private void checkState() {
		...
		superCarol = character.airSpeed == 12;
			...
			if (opponentNear) {
				...
				//Juggling
					...
					if (!shieldStun && freezeFrames <= 1 && (defended || parried) && !opponentProjectileComing) {
						...
						if (parried) {
							if (opponentRising) {
								combos.startCombo(1); //rising fair
							}
							else {
								if (superCarol && magic > 0 && framesToPunish(true) > 11) {
									combos.startCombo(7); //note to death
								}
								if ((distToFrontWall > 500 || magic < 2) && framesToPunish(true) > 16) {
									dashAttack();
								}
								else {
									combos.startCombo(5); //rising bair	
								}
								
							}
						}
						else {
							if (framesToPunish(true) > 11) {
								if (superCarol && magic > 0) {
									combos.startCombo(7);
								}
								else {
									combos.startCombo(5); //rising bair
						...
						//Counter, tube and super wings punish
							...
							if (framesToPunish(false) > 16) {
								dashAttack();
							}
							else if (framesToPunish(false) > 11) {
								if (magic > 2) {
									sideSpecial();
								}
								else {
				...
				//Neutral: opponent near and not attacking
					...
					//counter attack oos (for bruno up B)
						...
						if (parried) {
							if (opponentRising) {
								combos.startCombo(0); //rising fair
							}
							else {
								if (distToFrontWall > 500 || magic < 2) {
									dashAttack();
								}
								else {
									combos.startCombo(5); //rising bair	
